...
@section('title', 'Collections | E-commerce Store')
        ...
        <div class="card-body">
            <div class="row justify-content-between align-items-center">
                <div class="col-sm-auto mb-2 mb-sm-0">
                    <h6 class="mb-0">DashBoard > Collections</h6>
                </div>
            ...
            <div class="card-header">
                <div class="row align-items-center justify-content-between">
                    <div class="col-6 col-sm-auto d-flex align-items-center pr-0">
                        <h5 class="fs-1 mb-0 text-nowrap py-2 py-xl-0">Collections List</h5>
                    </div>
                ...
                <div class="dashboard-data-table">
                    <table class="table table-sm table-dashboard fs--1 data-table border-bottom" id='recordsTable' data-options='{"responsive":false,"pagingType":"simple","lengthChange":false,"searching":false,"pageLength":8,"columnDefs":[{"targets":[0,6],"orderable":false}],"language":{"info":"_START_ to _END_ Items of _TOTAL_ — <a href=https://prium.github.io/"#!\" class=\"font-weight-semi-bold\"> view all <span class=\"fas fa-angle-right\" data-fa-transform=\"down-1\"></span> </a>"},"buttons":["copy","excel"]}'>
                        ...
                        <tr>
                            ...
                            <th class="sort pr-1 align-middle text-center">Slug</th>
            ...
            <div class="modal-content border-0">
                <div class="modal-header px-5 text-white position-relative modal-shape-header">
                    <div class="position-relative z-index-1">
                        <div>
                            <h4 class="mb-0 text-white" id="authentication-modal-label">Add New Attribute</h4>
                        </div>
                ...
                <div class="modal-body py-4 px-5">
                    <form action="{{ route('collections.store') }}" method="POST" id="category">
                        @csrf
                        <div class="form-group">
                            <label for="name">Attribute Name</label>
                            <input class="form-control @error('name') is-invalid @enderror" type="text" id="name" name="name" placeholder="Type your attribute name..."/>
                            @error('name')
                        ...
                        <div class="form-group">
                            <button class="btn btn-primary btn-block mt-3" type="submit">Publish</button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div class="modal-dialog mt-6" role="document">
            <div class="modal-content border-0">
                <div class="modal-header px-5 text-white position-relative modal-shape-header">
                    <div class="position-relative z-index-1">
                        <div>
                            <h4 class="mb-0 text-white" id="authentication-modal-label">Edit Brand</h4>
                        </div>
                    </div><button class="close text-white position-absolute t-0 r-0 mt-1 mr-1" data-dismiss="modal" aria-label="Close"><span class="font-weight-light" aria-hidden="true">&times;</span></button>
                </div>
                <div class="modal-body py-4 px-5">
                    <form action="{{ route('admin.collections.update') }}" method="POST" id="editUpdate" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="id" id="categoryId">
                        <div class="form-group">
                            <label for="editName">Collection Name</label>
                            <input class="form-control" type="text" id="editName" name="name" placeholder="Type your attribute name..."/>
                        </div>
                        <div class="form-group">
...
@section('admin-js')
    ...
    <script>
            ...
            $("#category").validate({
                ...
                messages: {
                    name: {
                        ...
                        maxlength: "The brand name field 55 characters long"
                    }
                },
            });
        });

        //Attribute Edit Request
        ...
        var cId = [];
        function Edit(id) {
            axios.get('/admin/collections/' + id + '/edit')
                .then(response => {
                    var i = response.data;
                    cName.value = i.name;
                    cid.value = i.id;
                ...
                if(data_arr.length > 0){
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            //Delete Request Send
                            axios.post('/admin/all-collections', { id: data_arr})
                                .then(response => {
        ...
        function singleDelete(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    //Delete Request Send
                    axios.delete('/admin/attribute/' + id)
                        .then(response => {
                            $("#value_"+id).remove();
                            Swal.fire(
                                'Deleted!',
